...
RSpec.describe User, type: :model do
  ...
  context 'ユーザー新規登録できない場合' do
    ...
    it 'passwordが空だと登録できない' do
      user = FactoryBot.build(:user, password: '', password_confirmation: '00000000')
      user.valid?
    ...
    it '名字カナが全角カタカナであること' do
      user = FactoryBot.build(:user, last_name_kana: 'ﾀﾅｶ')
      user.valid?
    ...
    it '名前カナが全角カタカナであること' do
      user = FactoryBot.build(:user, first_name_kana: 'ﾀﾛｳ')
      user.valid?
    ...
    it '全角文字を含むパスワードでは登録できない' do
      user = FactoryBot.build(:user, password: 'パスワード123', password_confirmation: 'パスワード123')
      user.valid?
      expect(user.errors.full_messages).to include("Password must include at least one letter and one digit")
    end
